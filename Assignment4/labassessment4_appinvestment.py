...
Task #01: Create the function calculate_gains, which returns the total gains, total amount invested updated with the return on investment, and the gain margin.
...
Task #02: Implement the calculate_gains_over_time function to calculate the total amount earned over a period of time.




'''
multiplier_amount = 1000000 # global variable to store the multiplier amount

def calculate_gains(amount_inv=0.0):
    base_margin = 0.001
    gain_margin = base_margin
    total_gains = 0
    total_amount = amount_inv

    if amount_inv > 1000: #check if the amount is greater than the minimum value
        if amount_inv > multiplier_amount: #check if the amount is greater than the multiplier amount
            margin_div = int(amount_inv // multiplier_amount) #gather the value of the division
            gain_margin = base_margin + margin_div * 0.01 #update the gain margin by the multiplier mod

        total_gains = gain_margin * amount_inv #calculate the total amount of gains
        total_amount = amount_inv + total_gains #calculate the total amount plus the gain margin
    else:
        gain_margin = 0
        total_gains = 0
        total_amount = amount_inv
...
def calculate_gains_over_time(amount_inv=0.0, period=12): #function to calculate the total amount earned over a period of time

    _, total_amount, _ = calculate_gains(amount_inv) #calculating the first period before entering the loop 
    new_amount = total_amount 

    #looping over each period
    for i in range(1, period):  #startin at 1 since the first period is already calculated
